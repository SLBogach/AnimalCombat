        ...
        BattleSummary terminalSummary)
    {
        ...
        return eventType switch
        {
            ...
            nameof(CombatEventType.AttackPrepared) => new AttackPreparedPayload(
                ...
                ReadFighterId(payload.GetProperty("target_fighter_id"))),
            nameof(CombatEventType.ConflictResolved) => new ConflictResolvedPayload(
                related,
                new ExternalId(payload.GetProperty("conflict_id").GetString()!),
                new ExternalId(payload.GetProperty("intent_a_id").GetString()!),
                new ExternalId(payload.GetProperty("intent_b_id").GetString()!),
                new StableId(payload.GetProperty("category_a").GetString()!),
                new StableId(payload.GetProperty("category_b").GetString()!),
                payload.GetProperty("priority_a").GetInt32(),
                payload.GetProperty("priority_b").GetInt32(),
                ReadNullable(payload.GetProperty("winner_intent_id"), static value => new ExternalId(value)),
                Enum.Parse<ConflictResolutionResult>(payload.GetProperty("result").GetString()!),
                Enum.Parse<ConflictTieBreakMethod>(payload.GetProperty("tie_break_method").GetString()!)),
            ...
            nameof(CombatEventType.AttackHit) => new AttackHitPayload(
                ...
                payload.GetProperty("attack_tags").EnumerateArray()
                    .Select(item => new StableId(item.GetString()!))),
            nameof(CombatEventType.AttackMissed) => new AttackMissedPayload(
                related,
                new ExternalId(payload.GetProperty("impact_id").GetString()!),
                new ExternalId(payload.GetProperty("hit_group_id").GetString()!),
                Enum.Parse<AttackMissReason>(payload.GetProperty("miss_reason").GetString()!),
                payload.GetProperty("gap").GetInt32(),
                payload.GetProperty("hit_range_min").GetInt32(),
                payload.GetProperty("hit_range_max").GetInt32()),
            nameof(CombatEventType.Blocked) => new BlockedPayload(
                related,
                new ExternalId(payload.GetProperty("impact_id").GetString()!),
                new StableId(payload.GetProperty("defense_action_id").GetString()!),
                payload.GetProperty("chance_fp").GetInt32(),
                payload.GetProperty("damage_reduction_fp").GetInt32(),
                payload.GetProperty("guard_break").GetBoolean(),
                payload.GetProperty("cancelled_intent_ids").EnumerateArray()
                    .Select(item => new ExternalId(item.GetString()!))),
            nameof(CombatEventType.Dodged) => new DodgedPayload(
                related,
                new ExternalId(payload.GetProperty("impact_id").GetString()!),
                new StableId(payload.GetProperty("defense_action_id").GetString()!),
                payload.GetProperty("chance_fp").GetInt32(),
                payload.GetProperty("exit_position").GetInt32(),
                payload.GetProperty("cancelled_intent_ids").EnumerateArray()
                    .Select(item => new ExternalId(item.GetString()!))),
            nameof(CombatEventType.Countered) => new CounteredPayload(
                related,
                new ExternalId(payload.GetProperty("impact_id").GetString()!),
                new StableId(payload.GetProperty("counter_action_id").GetString()!),
                new StableId(payload.GetProperty("matched_tag").GetString()!),
                payload.GetProperty("cancelled_intent_ids").EnumerateArray()
                    .Select(item => new ExternalId(item.GetString()!))),
            nameof(CombatEventType.DamageApplied) => ReadDamageApplied(related, payload),
            nameof(CombatEventType.ResourceChanged) => new ResourceChangedPayload(
                related,
                Enum.Parse<ResourceKind>(payload.GetProperty("resource_kind").GetString()!),
                ReadNullable(payload.GetProperty("resource_id"), static value => new StableId(value)),
                payload.GetProperty("before").GetInt32(),
                payload.GetProperty("delta").GetInt32(),
                payload.GetProperty("after").GetInt32(),
                payload.GetProperty("minimum").GetInt32(),
                payload.GetProperty("maximum").GetInt32(),
                ReadEnum<ResourceClampReason>(payload.GetProperty("clamp_reason"))),
            ...
            nameof(CombatEventType.KnockbackApplied) => new KnockbackAppliedPayload(
                related,
                payload.GetProperty("from_position").GetInt32(),
                payload.GetProperty("to_position").GetInt32(),
                payload.GetProperty("requested_move").GetInt32(),
                payload.GetProperty("actual_move").GetInt32(),
                payload.GetProperty("blocked_by_wall").GetInt32()),
            nameof(CombatEventType.WallImpact) => new WallImpactPayload(
                related,
                Enum.Parse<MovementDirection>(payload.GetProperty("wall_side").GetString()!),
                payload.GetProperty("blocked_distance").GetInt32(),
                payload.GetProperty("threshold_met").GetBoolean(),
                payload.GetProperty("wall_damage").GetInt32(),
                payload.GetProperty("wall_stagger").GetInt32()),
            nameof(CombatEventType.GrabStarted) => new GrabStartedPayload(
                related,
                new ExternalId(payload.GetProperty("grab_id").GetString()!),
                ReadRequiredFighterId(payload.GetProperty("grabber_id")),
                ReadRequiredFighterId(payload.GetProperty("grabbed_id")),
                payload.GetProperty("hold_max_ticks").GetInt32(),
                Enum.Parse<GrabPriorityResult>(payload.GetProperty("priority_result").GetString()!)),
            nameof(CombatEventType.GrabEnded) => new GrabEndedPayload(
                related,
                new ExternalId(payload.GetProperty("grab_id").GetString()!),
                Enum.Parse<GrabEndReason>(payload.GetProperty("end_reason").GetString()!),
                ReadNullable(payload.GetProperty("throw_action_id"), static value => new StableId(value)),
                payload.GetProperty("grabber_position").GetInt32(),
                payload.GetProperty("grabbed_position").GetInt32()),
            ...
            nameof(CombatEventType.DrawDeclared) => new DrawDeclaredPayload(
                related,
                Enum.Parse<DrawReason>(payload.GetProperty("draw_reason").GetString()!),
                payload.GetProperty("participant_ids").EnumerateArray().Select(ReadRequiredFighterId),
                ReadNullable(payload.GetProperty("simultaneous_group_id"), static value => new ExternalId(value))),
