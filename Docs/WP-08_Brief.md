...
> Статус: `PREPARED / DECISIONS PROPOSED; IMPLEMENTATION NOT STARTED`.
>
> Все `OPEN-WP08-*` ниже имеют статус `OPEN / PROPOSED`. Они становятся `CLOSED` только после явного review/approval. До этого production-код, тесты, версии и fixtures WP-08 не изменяются.
...
Proposed golden uses generated canonical config with only three explicit setting overrides: `global.arena.start_position_a=4000`, `global.arena.start_position_b=5540`, `battle.time_limit_ticks=1`. All other settings, including event/watchdog caps, remain canonical. Exact request/profile:
...
## 11. План реализации после approval
...
## 12. Gate перед реализацией

WP-08 можно перевести из `PREPARED` в `READY`, когда:

- owner явно принимает или корректирует `OPEN-WP08-01..17`;
- все принятые решения помечены `CLOSED` в [Decisions.md](./Decisions.md);
...
- нет требования менять `UnityClient` или исторические replay bytes.

До approval этот документ является implementation proposal, а не доказательством реализованного поведения.
